...
class ScraperUtils:
    ...
    def http_get(self, url, check_errors=True, **kwargs):
        ...
        The response of the get request gets saved to the `response` attribute.
    ...
    def prepare_payload(self, html=None, nav_element=None):
        ...
        # parse out all the textareas within the form
        textarea_tags = form.find_all('textarea')
        for textarea_tag in textarea_tags:
            if textarea_tag.get('name') is not None:
                textarea_name = textarea_tag['name']
                textarea_value = textarea_tag.get('value', textarea_tag.get_text() or '')
                payload[textarea_name] = textarea_value
