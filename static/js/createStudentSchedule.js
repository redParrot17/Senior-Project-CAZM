		  ...
		  courses.forEach((course, index) => {
		    holderContents += `<div class="col-6 drag_box">
		            <div class="drag_container rounded border" ondrop="drop(event)" ondragover="allowDrop(event)">
									<div id="${course.course_code}-${course.semester}-${course.year}" coursecode="${course.course_code}" year="${course.year}" semester="${course.semester}" class="drag_item drag_item_fill" draggable="true" ondragstart="set_valid_drag_locations(event);drag(event);" ondragend="revert_drag_locations(event);dragEnd(event);" data-toggle="tooltip" data-delay="0" title="${getNameByCode(course.course_code)}">
		  ...
		  courses.forEach((course, index) => {
			...
			if(creditCount >17){
			  ...
			  <fieldset class="container scheduleContainer rounded" data-semester="${semester}" data-year="${year}" data-order="${semesterOrder}">
			    <legend class = "mb-0"> ${semester} ${year}</legend>
						<div class = "row">
							<h6 class = "pl-3" id = "${semester}-${year}-credits"> <i data-toggle="tooltip" data-delay="0" title="Credit Count High: There will be an added fee" class="fas fa-exclamation-triangle"></i> Scheduled Credits: ${creditCount} </h6>
						</div>
			      <div class="row rounded mb-2 drag_box_container mt-0" id="${semester}-${year}">` + holderContents;
			}
				...
				<fieldset class="container scheduleContainer rounded" data-semester="${semester}" data-year="${year}" data-order="${semesterOrder}">
					<legend class = "mb-0"> ${semester} ${year}</legend>
						<div class = "row">
							<h6 class = "pl-3" id = "${semester}-${year}-credits"> <i data-toggle="tooltip" data-delay="0" title="Credit Count Low: Need more credits for full time" class="fas fa-exclamation-triangle"></i> Scheduled Credits: ${creditCount} </h6>
						</div>
						<div class="row rounded mb-2 drag_box_container mt-0" id="${semester}-${year}">` + holderContents;
			}
			else{
